...
public class PDFExportHandler
{
    ...
    public async Task GeneratePDF(byte[] src, Stream dest, IEnumerable<Entry> rows, Settings settings, int? week, int? year)
    {
        try
        {
    ...
    private void WriteData(ASBNPdfFields fieldDate, ASBNPdfFields fieldNote, ASBNPdfFields fieldHours, ASBNPdfFields fieldLocation, Entry row, PdfDocument document)
    {
        ...
        // TODO: Fix this only being the ID for now, should load the matching workLocation


        FillField(document, fieldLocation, row.LocationId.ToString());
    }
