...
public static class MainViewWeekHelper
{
    ...
    /// </summary>
    /// <param name="entries">List of <see cref="Entry"/>s from the data source.</param>
    /// <param name="selectedWeek">The selected week of the year.</param>
    /// <param name="selectedYear">The selected year.</param>
    /// <returns>A List of <see cref="Entry"/>s, ready to display.</returns>
    public static List<Entry> GetCompleteWeek(List<Entry> entries, int? selectedWeek, int? selectedYear)
    {
        ...
        var dict = new Dictionary<DateTime, Entry?>();
        dict = possibleDates.ToDictionary(e => e.Date, e => (Entry)null);
        ...
        foreach(KeyValuePair<DateTime, Entry?> entry in dict)
        {
            var e = (entry.Value == null)
                ? new Entry { Date = DateTime.SpecifyKind(entry.Key, DateTimeKind.Local) }
                : entry.Value;
